...
# =========================================================
#   KyraPanel Manager v1.4 (FORCE CodeSandbox Detection)
# =========================================================
...
# FORCE CodeSandbox Detection
if [ -n "$CODESANDBOX_SBOX_ID" ] || [ "$HOME" = "/home/coder" ] || [ -d "/workspace" ] || [ -f "/.codesandbox.json" ]; then
    IS_CODESANDBOX=true
    echo -e "${BLUE}📦 CodeSandbox Environment Detected!${NC}"
else
...
do_install() {
    clear
    echo -e "${BLUE}=========================================================${NC}"
    echo -e "${BLUE}  🚀 Installing KyraPanel...${NC}"
...
PKG
    ...
    # server.js
    cat > server.js << 'SRV'
const express=require("express"),sqlite3=require("sqlite3").verbose(),cors=require("cors"),bodyParser=require("body-parser"),{v4:uuidv4}=require("uuid"),path=require("path"),http=require("http"),app=express(),PORT=process.env.PORT||6767;
...
db.serialize(()=>{db.run(`CREATE TABLE IF NOT EXISTS eggs(id TEXT PRIMARY KEY,name TEXT,docker_image TEXT,startup_cmd TEXT,variables TEXT)`);db.run(`CREATE TABLE IF NOT EXISTS servers(id TEXT PRIMARY KEY,name TEXT,egg_id TEXT,status TEXT,port INTEGER,created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`)});
app.get("/api/eggs",(req,res)=>db.all("SELECT * FROM eggs",[],(err,rows)=>res.json(err?[]:rows)));
app.post("/api/eggs",(req,res)=>{const{name,docker_image,startup_cmd,variables}=req.body;const id=uuidv4();db.run(`INSERT INTO eggs VALUES(?,?,?,?,?)`,[id,name,docker_image,startup_cmd,JSON.stringify(variables||[])],function(err){res.json(err?{error:err.message}:{success:true,id:id})})});
...
app.post("/api/servers",(req,res)=>{const{name,egg_id,port}=req.body;const id=uuidv4();db.run(`INSERT INTO servers(id,name,egg_id,status,port)VALUES(?,?,?,?,?)`,[id,name,egg_id,'installing',port],function(err){if(err)return res.status(500).json({error:err.message});db.get(`SELECT * FROM eggs WHERE id=?`,[egg_id],(err,egg)=>{if(!egg){db.run(`UPDATE servers SET status='error' WHERE id=?`,[id]);return res.json({success:true,server_id:id,warning:'Egg not found'})}const postData=JSON.stringify({name:`kyra_${id.substring(0,8)}`,image:egg.docker_image,port:port,startup:egg.startup_cmd,env:[]});const options={hostname:'localhost',port:6868,path:'/create',method:'POST',headers:{'Content-Type':'application/json','Content-Length':Buffer.byteLength(postData)}};const reqDaemon=http.request(options,(resD)=>{let data='';resD.on('data',c=>data+=c);resD.on('end',()=>{try{const result=JSON.parse(data);if(result.success){db.run(`UPDATE servers SET status='online' WHERE id=?`,[id]);res.json({success:true,server_id:id,daemon:result})}else{db.run(`UPDATE servers SET status='error' WHERE id=?`,[id]);res.json({success:false,error:result.error})}}catch(e){db.run(`UPDATE servers SET status='error' WHERE id=?`,[id]);res.json({success:false,error:'Invalid daemon response'})}})});reqDaemon.on('error',(e)=>{db.run(`UPDATE servers SET status='error' WHERE id=?`,[id]);res.json({success:false,error:'Daemon unreachable: '+e.message})});reqDaemon.write(postData);reqDaemon.end()})})});
app.delete("/api/servers/:id",(req,res)=>{db.get(`SELECT * FROM servers WHERE id=?`,[req.params.id],(err,server)=>{if(err||!server)return res.status(404).json({error:'Server not found'});const postData=JSON.stringify({name:`kyra_${req.params.id.substring(0,8)}`});const options={hostname:'localhost',port:6868,path:'/delete',method:'POST',headers:{'Content-Type':'application/json','Content-Length':Buffer.byteLength(postData)}};const reqDaemon=http.request(options,(resD)=>{let data='';resD.on('data',c=>data+=c);resD.on('end',()=>{db.run(`DELETE FROM servers WHERE id=?`,[req.params.id]);res.json({success:true})})});reqDaemon.on('error',()=>{db.run(`DELETE FROM servers WHERE id=?`,[req.params.id]);res.json({success:true,warning:'Server deleted from DB but daemon unreachable'})});reqDaemon.write(postData);reqDaemon.end()})});
app.get("/api/node-status",(req,res)=>{http.get('http://localhost:6868/system',(r)=>{let d='';r.on('data',c=>d+=c);r.on('end',()=>{try{res.json(JSON.parse(d))}catch(e){res.json({status:'offline'})}})}).on('error',()=>res.json({status:'offline'}))});
...
DPKG
    ...
    cat > daemon.js << 'DMN'
const express=require('express'),app=express();app.use(express.json());let mockContainers=[];const isCodeSandbox=process.env.CODESANDBOX_SBOX_ID!==undefined||process.env.HOME==='/home/coder';let docker;if(!isCodeSandbox){try{docker=require('dockerode')()}catch(e){isCodeSandbox=true}}app.get('/system',async(req,res)=>{if(isCodeSandbox){res.json({status:'online',memory:'2GB',containers:mockContainers.length,running:mockContainers.filter(c=>c.running).length,mock:true})}else{try{const info=await docker.info();res.json({status:'online',memory:Math.round(info.MemTotal/1024/1024/1024)+'GB',containers:info.Containers,running:info.ContainersRunning})}catch(e){res.status(500).json({error:e.message})}}});app.post('/create',async(req,res)=>{const{name,image,port,startup,env}=req.body;if(isCodeSandbox){mockContainers.push({id:'mock_'+Date.now(),name,image,port,running:true});res.json({success:true,id:'mock_'+Date.now(),mock:true})}else{try{const stream=await docker.pull(image);await new Promise((resolve,reject)=>docker.modem.followProgress(stream,(err,out)=>err?reject(err):resolve(out)));const container=await docker.createContainer({Image:image,name:name,ExposedPorts:{[`${port}/tcp`]:{},[`${port}/udp`]:{}},HostConfig:{PortBindings:{[`${port}/tcp`]:[{HostPort:port.toString()}],[`${port}/udp`]:[{HostPort:port.toString()}]},RestartPolicy:{Name:'always'}},Env:env||[],Cmd:startup?startup.split(' '):undefined});await container.start();res.json({success:true,id:container.id})}catch(e){res.status(500).json({error:e.message})}}});app.post('/delete',async(req,res)=>{const{name}=req.body;if(isCodeSandbox){mockContainers=mockContainers.filter(c=>c.name!==name);res.json({success:true})}else{try{const container=docker.getContainer(name);await container.stop();await container.remove();res.json({success:true})}catch(e){res.status(500).json({error:e.message})}}});app.listen(6868,'0.0.0.0',()=>console.log('🚀 KyraDaemon running on port 6868'));
DMN
...
HTML3
    ...
    cd "$INSTALL_DIR" && npm install --production >/dev/null 2>&1
    cd "$DAEMON_DIR" && npm install --production >/dev/null 2>&1
    ...
    echo -e "${GREEN}🌐 Panel URL: Port 6767${NC}"
    echo -e "${GREEN}🤖 Daemon URL: Port 6868${NC}"
    echo -e "${GREEN}=========================================================${NC}"
}

# =========================================================
#  AUTO-INSTALL IF CODESANDBOX (NO MENU)
# =========================================================
...
# =========================================================
echo -e "${BLUE}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║              KyraPanel Manager v1.4                  ║${NC}"
echo -e "${BLUE}╠══════════════════════════════════════════════════════════╣${NC}"
...
case $choice in
    1) do_install ;;
    2) echo "Uninstall..."; rm -rf "$INSTALL_DIR" "$DAEMON_DIR"; echo "✅ Done" ;;
    3) do_install ;;
    4) exit 0 ;;
    *) echo "Invalid"; exit 1 ;;
esac
