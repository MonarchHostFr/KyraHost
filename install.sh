...
<main class="flex-grow container mx-auto p-4">
  <div id="login-view" class="max-w-md mx-auto mt-10 bg-gray-800 p-6 rounded-lg shadow">
    ...
    <form onsubmit="handleLogin(event)" class="space-y-4">
      <input type="text" id="login-user" placeholder="Username" class="w-full p-3 rounded bg-gray-700 border border-gray-600" required>
      <input type="password" id="login-pass" placeholder="Password" class="w-full p-3 rounded bg-gray-700 border border-gray-600" required>
      <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 py-3 rounded font-bold">Login</button>
    </form>
    <p class="mt-4 text-sm text-center text-gray-400">No account? <a href="#" onclick="showView('register-view')" class="text-blue-400">Register</a></p>
  ...
  <div id="register-view" class="hidden max-w-md mx-auto mt-10 bg-gray-800 p-6 rounded-lg shadow">
    ...
    <form onsubmit="handleRegister(event)" class="space-y-4">
      <input type="text" id="reg-user" placeholder="Username" class="w-full p-3 rounded bg-gray-700 border border-gray-600" required>
      <input type="password" id="reg-pass" placeholder="Password" class="w-full p-3 rounded bg-gray-700 border border-gray-600" required>
      <button type="submit" class="w-full bg-green-600 hover:bg-green-700 py-3 rounded font-bold">Register</button>
    </form>
    <p class="mt-4 text-sm text-center text-gray-400">Have an account? <a href="#" onclick="showView('login-view')" class="text-blue-400">Login</a></p>
  </div>
  <div id="admin-view" class="hidden space-y-6">
    <h2 class="text-3xl font-bold text-blue-400">Admin Dashboard</h2>
    <div class="bg-gray-800 p-6 rounded-lg shadow">
      <h3 class="text-xl font-bold mb-4">🥚 Import Pterodactyl Egg (.json)</h3>
      <form onsubmit="importEgg(event)" class="space-y-4">
        <textarea id="egg-json" rows="6" placeholder='Paste Pterodactyl Egg JSON here...' class="w-full p-3 rounded bg-gray-700 border border-gray-600 font-mono text-sm" required></textarea>
        <button type="submit" class="bg-purple-600 hover:bg-purple-700 px-6 py-2 rounded font-bold">Import Egg</button>
      </form>
    </div>
    <div class="bg-gray-800 p-6 rounded-lg shadow">
      <h3 class="text-xl font-bold mb-4">🖥️ Create Server for User</h3>
      <form onsubmit="createServer(event)" class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <select id="admin-user-select" class="p-3 rounded bg-gray-700 border border-gray-600" required><option value="">Select User</option></select>
        <select id="admin-egg-select" class="p-3 rounded bg-gray-700 border border-gray-600" required><option value="">Select Egg</option></select>
        <input type="text" id="srv-name" placeholder="Server Name" class="p-3 rounded bg-gray-700 border border-gray-600" required>
        <input type="number" id="srv-port" placeholder="Port" class="p-3 rounded bg-gray-700 border border-gray-600" required>
        <button type="submit" class="md:col-span-4 bg-blue-600 hover:bg-blue-700 py-3 rounded font-bold">Create Server</button>
      </form>
    </div>
    <div class="bg-gray-800 p-6 rounded-lg shadow">
  ...
  <div id="user-view" class="hidden">
    ...
    <div id="user-server-list" class="grid grid-cols-1 md:grid-cols-2 gap-4"></div>
  </div>
</main>
<script>
...
async function loadAdminData() {
  ...
  document.getElementById('admin-server-list').innerHTML = servers.length ? servers.map(s => '<div class="bg-gray-700 p-3 rounded flex justify-between items-center"><div><span class="font-bold">' + s.name + '</span> <span class="text-gray-400 text-sm">(Port: ' + s.port + ' | User: ' + (s.username || 'Unknown') + ')</span></div></div>').join('') : '<p class="text-gray-400">No servers yet.</p>';
}
...
async function loadUserServers() {
  const servers = await (await fetch('/api/servers', { headers: { 'Authorization': 'Bearer ' + token } })).json();
  document.getElementById('user-server-list').innerHTML = servers.length ? servers.map(s => '<div class="bg-gray-800 p-6 rounded-lg shadow border-l-4 border-green-500"><h3 class="text-xl font-bold">' + s.name + '</h3><p class="text-gray-400 mt-1">Port: ' + s.port + '</p><div class="mt-4"><span class="text-green-400 font-semibold">' + s.status + '</span></div></div>').join('') : '<p class="text-gray-400 bg-gray-800 p-6 rounded-lg">You have no servers.</p>';
}
checkAuth();
</script></body></html>
EOF
    ...
    echo "🌐 Panel URL: http://localhost:6767"
...
do_install_cf() {
    ...
    chmod +x cloudflared
    sudo mv cloudflared /usr/local/bin/ 2>/dev/null || mv cloudflared ~/bin/ 2>/dev/null || echo "⚠️ Moved to current directory"
    echo "✅ Cloudflared installed!"
    echo "💡 Run: cloudflared tunnel --url http://localhost:6767"
}
...
# ==========================================
# NEW FRESH INSTALL FUNCTION
# ==========================================
do_fresh_install() {
    echo ""
    echo "⚠️  WARNING: FRESH INSTALL WILL DELETE ALL DATA!"
    echo "   (Users, Servers, Eggs, and Database will be wiped)"
    echo "========================================================"
    ...
    if [[ $confirm == [yY] || $confirm == [yY][eE][sS] ]]; then
        ...
        sleep 1
        rm -rf "$INSTALL_DIR" "$DAEMON_DIR"
        echo "✅ Wipe complete. Starting fresh installation..."
        do_install
    else
...
if [ "$IS_CODESANDBOX" = true ]; then
    echo "📦 CodeSandbox detected! Auto-installing KyraPanel..."
    sleep 2
    do_install
    exit 0
fi

# For regular terminals (VPS), show menu
while true; do
    clear
    echo "========================================================"
    echo "           🐉 KyraPanel Unified Manager               "
    echo "========================================================"
    echo "  1) Install / Update KyraPanel (Preserves Database)"
    echo "  2) Uninstall KyraPanel (Removes files)"
    echo "  3) Fresh Install (Wipes ALL data and reinstalls)"
    ...
    case $choice in
        ...
        2) do_uninstall; read -p "Press Enter to return to menu..." ;;
        3) do_fresh_install; read -p "Press Enter to return to menu..." ;;
